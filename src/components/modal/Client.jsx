...
export default function ModalClient({ open, setOpen, modalData }) {
            ...
            <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
              <div className=" flex flex-col">
                <div className=' relative h-56 w-full'>
                    <Image src={modalData?.image} alt={modalData?.subHeading} className=' absolute object-contain shadow-lg' fill />
                </div>
                <div className=' mt-5 text-[#045862]'>
                    <h2 className=' font-robotoMedium '>{modalData?.heading}</h2>
                    <p className=' font-robotoRegular'>{modalData?.subHeading}</p>
                </div>
              </div>
            </div>
